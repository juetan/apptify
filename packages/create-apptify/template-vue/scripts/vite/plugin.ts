...
const getBuildInfo = async () => {
  ...
  const version = commits ? `${latestTag}.${commits}` : pkg.version;
  const content = ` 欢迎访问!    版本：${version}  标识：${hash}  构建日期：${time}`;
...
export default function plugin(): Plugin {
  ...
  return {
    name: "vite-plugin-self-plugin",
    enforce: "pre",
    configResolved(resolvedConfig) {
      config = resolvedConfig;
      const defaultExt = config.mode === "production" ? "prod" : "dev";
